...
const run = async function main() {
    console.log('🧪 Verifying Shop End-to-End Flow...');

    // Initialize DB to verify status later
    try {
        const dbPath = process.env.PERSISTENCE_DB_PATH || path.join(__dirname, '../data/persistence.db');
        DatabaseManager.initialize({ path: dbPath });
    } catch (e: any) {
        console.warn('⚠️  Database initialization warning (might be already initialized or locked):', e.message);
    }
    ...
    try {
        ...
        if (search.data.length > 0) {
            ...
            console.warn('⚠️ No catalog items found. Import items first.');
            // Try to import one?
            // Skip for now, assume manual setup was done or initial seed.
            process.exit(1);
        }
    } catch (e: any) {
        console.error('❌ Failed to search catalog:', e.message);
        process.exit(1);
    }
    ...
    try {
        ...
        console.error('❌ Failed to create cart:', e.message);
        process.exit(1);
    }
    ...
    try {
        const res = await axios.post(`${API_URL}/api/wa/cart/${cartId}/checkout`, {
            ...
            // paymentMethod removed as it's not in the API schema
        });
        invoiceUrl = res.data.invoiceOfferUrl;
        ecocashRef = res.data.ecocashRef; // The one generated by the mock or API
        console.log(`✅ Checkout Successful. Invoice URL: ${invoiceUrl?.substring(0, 30)}...`);
        console.log(`ℹ️ EcoCash Ref: ${ecocashRef}`);
    } catch (e: any) {
    ...
    try {
        ...
        console.log(`ℹ️ Found Payment Record. Source Ref: ${sourceRef}`);
        ...
        console.error('❌ Payment Simulation failed:', e.message);
        process.exit(1);
    }
    ...
    try {
        ...
        if (cart.status === 'paid') {
            console.log(`✅ Cart Status Verified: PAID`);
        } else {
            console.error(`❌ Cart Status Mismatch: ${cart.status}`);
            process.exit(1);
        }
        ...
        if (receipt) {
            ...
            console.error(`❌ No Receipt Found for Cart`);
            process.exit(1);
        }
    ...
    console.log('🎉 Shop Flow Verified Successfully!');
};
