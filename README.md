...
<h1 align="center"><b>Credo Controller — W3C SSI REST API (Single-Tenant)</b></h1>
<p align="center">
  <a
...
Credo Controller is a W3C-first REST API for building single-tenant SSI agents with Credo. It focuses on did:key / did:jwk / did:web, OIDC4VC issuance, and DIDComm interop.
...
See the [docker compose file](https://github.com/hyperledger/aries-framework-javascript-ext/tree/main/docker-compose.yml) for an example of using the afj-rest image with Docker Compose V2.
...
No Indy prerequisites required. Start the REST server using the following command:

```sh
node ./bin/afj-rest.js start \
  --label "AFJ Rest" \
  --wallet-id "walletId" \
  --wallet-key "walletKey" \
  --endpoint http://localhost:5000 \
