 ...
 *
 * This layer is authoritative-status aware but intentionally fail-closed:
 * unavailable, malformed, unverifiable, or unsupported status data remains
 * `unknown` and cannot become a trusted business decision.
 */
export class CredentialStatusService implements CredentialStatusResolver {
  async resolve(input: {
    ...
    try {
      ...
      if (valid) return this.result('valid', true, 'credential_status_valid', entry)

      const message = entry.statusMessage?.find(({ status }) => status === String(statusValue))
      const status: CredentialStatus = entry.statusPurpose === 'suspension' ? 'suspended' : 'revoked'
      return this.result(status, true, message?.message ? 'credential_status_invalid' : 'credential_status_nonzero', entry)
    } catch (error) {
  ...
  private isConformingStatusListCredential(credential: BitstringStatusListCredential | string): boolean {
    if (typeof credential === 'string') {
      ...
      try {
        ...
        return this.hasExpectedType(payload.type ?? (payload.vc as Record<string, unknown> | undefined)?.type, 'BitstringStatusListCredential')
      } catch {
        return false
      }
    }
  ...
  private extractStatusList(credential: BitstringStatusListCredential | string): BitstringStatusList | undefined {
    if (typeof credential === 'string') {
      ...
      try {
        const payload = JSON.parse(Buffer.from(parts[1], 'base64url').toString('utf8')) as Record<string, unknown>
        return (payload.credentialSubject ?? (payload.vc as Record<string, unknown> | undefined)?.credentialSubject) as BitstringStatusList | undefined
      } catch {
        return undefined
      }
    }
