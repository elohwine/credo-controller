...
RUN node -e "try { const { ariesAskar } = require('@hyperledger/aries-askar-nodejs'); console.log('✓ Askar version:', ariesAskar.version()); } catch (e) { console.error('ERROR loading Askar:', e.message); process.exit(1); }" || \
...
# Create non-root user with a home directory for security and agent stability
RUN groupadd -r credo && useradd -r -g credo -m -d /home/credo credo && \
    mkdir -p /home/credo && chown -R credo:credo /home/credo
...
# Default command (run issuer + holder servers)
CMD ["sh", "-c", "node ./samples/startServer.js & node ./samples/startHolderServer.js & wait"]
